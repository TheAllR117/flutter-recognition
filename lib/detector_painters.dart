...
class FaceDetectorLandmarkPainter extends CustomPainter {
    ...
    for (String label in results.keys) {
      for (final Face face in results[label]) {
        ...
        if (Platform.isAndroid && !camPos) {
          canvas.drawPoints(
              ui.PointMode.points, contour!.positionsList.map((offset) => Offset(size.width - (offset.dx * scaleX), offset.dy * scaleY)).toList(), paint);
        } else if (Platform.isIOS || camPos) {
          canvas.drawPoints(ui.PointMode.points, contour!.positionsList.map((offset) => Offset((offset.dx * scaleX), offset.dy * scaleY)).toList(), paint);
        }
        for (final connection in faceMaskConnections) {
          if (Platform.isAndroid && !camPos) {
            // ignore: unnecessary_null_comparison
            if (contour!.positionsList != null && contour.positionsList.length != 0) {
              double tmpDxConnectionX = size.width - contour.positionsList[connection.x].scale(scaleX, scaleY).dx;
              double tmpDyConnectionX = contour.positionsList[connection.x].scale(scaleX, scaleY).dy;
              Offset a = Offset(tmpDxConnectionX, tmpDyConnectionX);
              double tmpDxConnectionY = size.width - contour.positionsList[connection.y].scale(scaleX, scaleY).dx;
              double tmpDyConnectionY = contour.positionsList[connection.y].scale(scaleX, scaleY).dy;
        ...
        textPainter.layout();
        if (Platform.isAndroid && !camPos) {
          textPainter.paint(canvas, new Offset(size.width - (face.boundingBox.left.toDouble()) * scaleX, (face.boundingBox.top.toDouble()) * scaleY));
        } else if (Platform.isIOS || camPos) {
          textPainter.paint(canvas, new Offset((face.boundingBox.left.toDouble()) * scaleX, (face.boundingBox.top.toDouble()) * scaleY));
        }
      }
    }
  }
  ...
  bool shouldRepaint(FaceDetectorLandmarkPainter oldDelegate) {
    return oldDelegate.absoluteImageSize != absoluteImageSize || oldDelegate.results != results;
  }
}
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (String label in results.keys) {
      for (Face face in results[label]) {
        ...
        if (Platform.isAndroid && !camPos) {
          canvas.drawRRect(
              _scaleRect(rect: face.boundingBox, imageSize: imageSize, widgetSize: size, scaleX: scaleX, scaleY: scaleY, cameraPosition: camPos), paint);
        } else if (Platform.isIOS || camPos) {
          canvas.drawRect(
            _scaleRect(
              rect: face.boundingBox,
