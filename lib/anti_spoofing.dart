...
class FaceAntiSpoofing {
  ...
  static final String modelFile = "FaceAntiSpoofing.tflite";
  static final inputImageSize =
  ...
  static final routeIndex = 6; // Route index observed during training
  static final laplaceThreshold = 50; // Laplace sampling threshold
  static final laplacianThreshold = 1000; // Picture clarity judgment threshold
  ...
  static String antiSpoofing(
    ...
    if (laplace1 < laplacianThreshold) {
      text = "$text, False";
    } else {
    ...
    if (laplace2 < laplacianThreshold) {
      text2 = "$text2, False";
    } else {
      // Live detection
  ...
  static Future loadSpoofModel() async {
    try {
      interpreter = await tfl.Interpreter.fromAsset(modelFile);

      print('**********\n Loaded successfully model $modelFile \n*********\n');
    } catch (e) {
  ...
  static double _antiSpoofing(imglib.Image bitmap) {
    ...
    imglib.Image bitmapScale =
        imglib.copyResizeCropSquare(bitmap, inputImageSize);
    ...
    input[0] = img.reshape([1, inputImageSize, inputImageSize, 3]);
    ...
    if (input.isNotEmpty &&
      ...
      interpreter.runForMultipleInputs(
          input as List<Object>, outputs as Map<int, Object>);
  ...
  dynamic leafScore2(var clssPred) {
    return clssPred[0][routeIndex];
  }
